...
let StorageInterface = {
	...
	createInterface: function(id, interface) {
		...
		if (tilePrototype) {
			...
			interface.checkSide = function(slotSideTag, side) {
				if (slotSideTag == undefined) return true;
				if (typeof slotSideTag == "number") return slotSideTag == side;
				if (slotSideTag == "horizontal" && side > 1) return true;
				let sideEnum = {
					down: 0,
					up: 1,
					north: 2,
	...
	getContainerSlots: function(container, mode, side) {
		...
		if (container.slots) {
			...
			let type = mode? container.getType() : 0;
			switch(type) {
			case 1:
			case 38:
			case 39:
